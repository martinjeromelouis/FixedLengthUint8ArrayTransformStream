...
// inputs. See aes_cbc_cryptostream deno third-party library for an implementation example.

import { BytesList } from "./deps.ts";

export class FixedLengthUint8ArrayTransformStream
  extends TransformStream<Uint8Array, Uint8Array> {
  /**
   * Constructor
   * @param {number} firstChunkLength - The length of the first chunk. If 0, the first chunk is handled like other chunks.
   * @param {number} otherChunksLength - The length of each chunk except the first one if firstChunkLength > 0 and the last one if its length is strictly inferior to otherChunksLength.
